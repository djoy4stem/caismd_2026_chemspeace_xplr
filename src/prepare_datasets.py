...
Run
---
    python scripts/prepare_datasets.py    (from workshop root, cheminf conda env)
"""
...
def validate_smiles(df):
    """Drop rows with invalid SMILES and canonicalise the survivors."""
    if not RDKIT_AVAILABLE:
        return df
    canonical = []
    valid_mask = []
    for s in df["SMILES"]:
        ...
        if mol is None:
            ...
            canonical.append(Chem.MolToSmiles(mol))   # canonical form
    ...
    df["SMILES"] = [s for s, v in zip(canonical, valid_mask) if v]
    return df
